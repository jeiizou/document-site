---
title: '[笔记]WebgGL高级编程-CH2'
date: 2019-01-25 21:41:09
mathjax: true
category:
    - 笔记
    - WebGL高级编程
...
> 创建和初始化webgl, 可以借助[webgl-util](https://github.com/kmindi/webgl-utils.js)这个库. 

### 调试WebGL应用程序

#### gl.getError()

当WebGL检测到代码中的一个错误时, 它会生成并记录错误代码. 当记录一个错误后, 新的错误就不会被记录, 直到调用`gl.getError()`, 这个方法用来查询记录下来的错误代码, 返回当前的错误代码, 并把当前错误复位为gl.NO_ERROR. 此后, 可以重新记录新的错误. 

在WebGL应用程序中使用gl.getError()方法时, 用户可以采用两种策略:

- 编写防御性代码时, 总是在自己的代码中插入gl.getError(). 当捕获一个错误是, 采取适当的方法进行处理. 这不是推荐的方法, 因为调用`gl.getError()`方法会影响应用程序的性能
- 另一个策略在WebGL中比较常见, 它只在调式程序时调用`gl.getError()`方法. 只有在开发中想要找出程序存在的问题时使用`gl.getError()`. 当程序准备交付使用时, 它不在包含任何`gl.Error()`方法调用. 

另外我们可以使用[webgl-debug.js](https://github.com/vorg/webgl-debug), 然后再创建WebGLRenderingContext对象时, 调用一下的函数, 把它封装到这个js中:
...
在上面的代码示例中, 顶点着色器和片元着色器都以字符串的形式插入到代码中, 此外还有更简单的方法, 就是把它插入到script标记中: 

```html
<script id="shader-vs" type="x-shader/x-vertex">
    attribute vec3 aVertexPositon;
    void main(){
